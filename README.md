...
| キャラクター立ち絵 (FLUX.2 Klein) | 同上だが蒸留モデルで高速。ラフ確認向け。**画面を開いたとき最初から選ばれています** |
| 背景 (SDXL) | 文章だけから背景を描く |
| 新規キャラクター (最大4枚の参照画像からブレンド) | 1〜4枚の参考画像(顔・服装・雰囲気など)の要素を混ぜて新しいキャラを作る。1枚目以外は省略可 |
| 既存キャラクターの表情・ポーズ違い | 保存済みキャラクター(または1枚の参照画像)を元に、同じキャラのまま表情やポーズだけを変える |
| 新規背景 (最大4枚の参照画像からブレンド) | 1〜4枚の参考画像(構図・色合い・雰囲気など)の要素を混ぜて新しい背景を作る。1枚目以外は省略可 |
...
1. 以下のリンクを開く(パソコンのブラウザ推奨)
   https://colab.research.google.com/github/jinziruixi07-design/image-studio/blob/main/colab/ImageStudio_Colab.ipynb
...
> **注意**: 「新規キャラクター」「既存キャラクターの表情・ポーズ違い」「新規背景」(参照画像を使う機能)は、
> ノートブックの見出し「6. (任意・上級者向け)」で `ENABLE_IPADAPTER = True` に変えて
> そのセルを実行しないと使えません(デフォルトはFalseで、追加ダウンロードをスキップする設定に
> なっています)。「Node 'IPAdapterUnifiedLoader' not found」のようなエラーが出たら、
> このセルを有効にし忘れています。有効にした後は、新しいセルで `comfyui_proc.terminate()` を
> 実行してから、見出し「7. ComfyUI をバックグラウンドで起動」のセルを再実行すれば反映されます
> (Image Studio側・URL発行側は再実行不要です)。

### コードを更新した後、Colabに反映されない場合

このリポジトリのコードを後から直しても、**すでに起動中のColabセッションには自動で反映されません**。
「前に直したはずのエラーがまだ出る」という場合は、Colabの「ランタイム」→「ランタイムを再起動」
→ 上から全セルを再実行、を試してください(モデルファイルはGoogleドライブに残るので2回目は速いです)。

### スマホで使う

Image Studioはただのブラウザページなので、**手順4で表示されたURLをスマホのブラウザで
開けば、スマホから直接プロンプト入力・生成・画像保存ができます**。パソコンを閉じても、
Colab側が動き続けている間はURLは有効です。

ただし、Colabノートブック自体(セルを実行する作業)はパソコンで行うのがおすすめです。
スマホでも開けますが操作しづらいためです。
...
プロンプト欄・ネガティブプロンプト欄とも**日本語でそのまま**書いてOKです。APIキーを設定していれば、
「生成前にAIが自動で英語プロンプトに変換する」にチェックが入った状態になっており、
生成ボタンを押すたびにClaude(Anthropic API)が日本語の雑な指示を画像生成向けの
英語プロンプトへ自動変換してから生成します(変換後の英語も欄に表示されるので、
次から使い回すこともできます)。チェックを外せば入力した文章がそのまま使われます。

プロンプト欄・ネガティブプロンプト欄それぞれの隣にある「AIで変換」ボタンは、生成はせず変換結果だけ先に
確認したいときに使います。**この機能(自動変換もボタンも)は無くてもサイトの他の
機能は全部使えます。**
